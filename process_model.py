...
def process_data_bag_of_words(X_train, y_train, X_test, y_test, stopwords = None, n_folds = 10):
    result = pd.DataFrame()
    
    """
    Processa os dados de treino e teste
    """
    #vectorizer = CountVectorizer(analyzer = "word", ngram_range=(1,2), stop_words = stopwords)#, lowercase=True)#, min_df = 1)
    vectorizer = TfidfVectorizer(sublinear_tf=True, use_idf=True,stop_words=stopwords, lowercase=True, ngram_range=(1,2))
